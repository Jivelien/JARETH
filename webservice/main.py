...
def get_all_users(current_user):
    ...
    with engine.connect() as conn: 
        results = conn.execute("SELECT username, public_id, mail, password FROM users")
    ...
    return Response(response = jsonify(users)
                    , status = 200
...
def create_user():
    ...
    if not data or not data.get('username') or not data.get('mail') or not data.get('password'):
        return Response(response="Missing informations in payload"
                        , status=400)

    engine = create_engine(app.config['SQLALCHEMY_DATABASE_URI'])

    with engine.connect() as conn: 
        result = conn.execute(f"SELECT 1 FROM users WHERE mail='{data.get('mail')}'").fetchone()

    if result:
        return Response(response="Mail address already used"
                        , status=400)
...
def get_user(current_user, public_id):
    ...
    if result:
        ...
        return Response(response = jsonify(user)
                        , status = 200
                        , mimetype = 'application/json')
    else:
        return Response(response = 'Unable to find user'
                        , status = 404)
...
def update_user(current_user, public_id):
    if not current_user == public_id:
        return Response(message = "Permission denied to update this user"
                        , status = 403)
    ...
    if result:
        ...
        return Response(response="No user to update"
                        , status=404)
...
def delete_user(current_user, public_id):
    if not current_user == public_id:
        return Response(response="Permission denied to delete this user"
                        , status=401)
...
def create_cigarette(current_user):
    ...
    try:
        event_time = datetime.datetime.strptime(data.get('event_time'), "%Y-%m-%d %H:%M:%S")
    except ValueError:
        return Response(status=400)

    engine = create_engine(app.config['SQLALCHEMY_DATABASE_URI'])
    with engine.connect() as conn: 
        result = conn.execute(f"SELECT 1 FROM users WHERE public_id='{current_user}' and event_time='{event_time}").fetchone()
    if result:
        return Response(status=400)
        
    with engine.begin() as conn: 
        conn.execute(f"INSERT INTO smoked_cigarettes(public_user_id, event_time) VALUES('{current_user}', '{event_time}')")
...
def get_cigarettes(current_user):
    engine = create_engine(app.config['SQLALCHEMY_DATABASE_URI'])
    with engine.connect() as conn: 
        results = conn.execute(f"SELECT cigarette_id, event_time FROM smoked_cigarettes WHERE public_user_id='{current_user}'")
    ...
    return Response(response=jsonify(cigarettes)
                    , status=200)
